<?php
	class plugin_kontakt_formular {
		...
		function formular() {
			$html  = "<!-- Plugin: Kontakt -->\n";
			$html .= "<form method=\"post\" action=\"" . get_page_link() . "\">\n";
		...
		function generateSpam() {
			...
			$html .= "<label>Spam Check:<strong>*</strong></label>\n";
			$html .= "<input type=\"text\" name=\"kontakt_spam\" value=\"\"" . self::htmlShort();
			$html .= "<img src=\"./wp-content/plugins/plugin-kontakt/functions/captcha.php\" alt=\"Spam Captcha\"" . self::htmlShort();
			$html .= "</div>";
			return $html;
		}
		...
		function checkFields() {
			...
			if(isset($_POST['kontakt_send']) && $_POST['kontakt_send'] != "") {
				$mail			= "Es ist eine neue Kontaktanfrage eingegangen:\n\n";
				$fields 		= $wpdb->get_results("SELECT * FROM `" . $wpdb->prefix . "kontakt_fields` ORDER BY `order` ASC");
				$errors 		= 0;
				$error	 		= array();
				$subject_mail	= "";
				...
				if($errors == 0) {
					self::changeNotify("");
					self::addNotify("Ihre Kontaktanfrage wurde erfolgreich versand.");
					...
					self::addNotify("Bitte korrigieren Sie Ihre eingaben:<br />");
